<?
  ...
  $chrom = $HTTP_POST_VARS["chrom"];
  $map = $HTTP_POST_VARS["map"];
  $coords = $HTTP_POST_VARS["coords"];
  $map_type = $HTTP_POST_VARS["maptype"];
...
<?  if($chrom == 2) { ?><p>You are evaluating chromosome 2 of tomato, the short arm of which is the location of the nucleolar organizer region and is totally heterochromatic.</p><? }
    if($cent_coord == -999)
...
<?
    ...
    $mult_factor = $HTTP_POST_VARS["factor"];
